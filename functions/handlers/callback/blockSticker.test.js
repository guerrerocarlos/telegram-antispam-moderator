...
describe('Block sticker', () => {
    beforeEach(done => {
        bot.mockClear();
        db.mockClear();
        const mockRequest = getMockData('blockStickerCallback');
    ...
    it('should delete forward message, control message and original message', () => {
        ...
        assert.strictEqual(25, messageId3);
    });
    it('should restrict chat member', () => {
        ...
        assert.strictEqual(30000, userId);
    ...
    it('should send success message', () => {
        ...
        assert.include(text, 'badstickerpack');
    });
    it('should add stickerpack to blacklist', () => {
        assert.strictEqual(1, db.blackListSticker.mock.calls.length);
        const [[{chatId, blockedString}]] = db.blackListSticker.mock.calls;

        assert.strictEqual(-1001245604961, chatId);
        assert.strictEqual('badstickerpack', blockedString);
    });
});