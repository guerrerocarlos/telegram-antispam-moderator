...
describe('Block domain', () => {
    beforeEach(done => {
        bot.mockClear();
        db.mockClear();
        const mockRequest = getMockData('blockDomainCallback');
    ...
    it('should send success message', () => {
        ...
        assert.include(text, 'google.com');
    });
    it('should add domain to blacklist', () => {
        assert.strictEqual(1, db.blackListDomain.mock.calls.length);
        const [[{chatId, blockedString}]] = db.blackListDomain.mock.calls;

        assert.strictEqual(-1001245604961, chatId);
        assert.strictEqual('google.com', blockedString);
    });
});
describe('Block domain - no http', () => {
    beforeEach(done => {
        bot.mockClear();
        db.mockClear();
        const mockRequest = getMockData('blockDomainCallbackNoHttp');
        handle(mockRequest).then(() => {
            done();
        });
    });
