...
describe('Block link', () => {
    beforeEach(done => {
        bot.mockClear();
        db.mockClear();
        const mockRequest = getMockData('blockLinkCallback');
    ...
    it('should add link to blacklist', () => {
        assert.strictEqual(1, db.blackListLink.mock.calls.length);
        const [[{chatId, blockedString}]] = db.blackListLink.mock.calls;

        assert.strictEqual(-1001245604961, chatId);
        assert.strictEqual('google.com/test1.php?test=3#2', blockedString);
    });
});